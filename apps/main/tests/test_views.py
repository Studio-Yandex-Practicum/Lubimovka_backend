...
class TestMainAPIViews:
    def test_get_main_fields(self, client, news, blog, banners, plays):
        ...
        for field in fields:
            assert field in response.data, f"Проверьте, что при GET запросе {MAIN_URL} " f"data содержит {field}"
    ...
    def test_get_main_first_screen_fields(self, client, news, blog, banners, plays):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert field in response.data["first_screen"], (
                f"Проверьте, что при GET запросе {MAIN_URL} data[first_screen] " f"" f"содержит {field}"
            )
    ...
    def test_get_main_news_and_short_list_fields(self, client, news, blog, banners, plays):
        ...
        for field in fields:
            for data_field in data_fields:
                assert field in response.data.get(data_field), (
                    f"Проверьте, что при GET запросе {MAIN_URL} " f"data[{data_field}] содержит {field}"
                )
    ...
    def test_get_main_afisha_fields(self, client, news, blog, banners, plays):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert field in response.data["afisha"], (
                f"Проверьте, что при GET запросе {MAIN_URL} data[afisha] " f"содержит {field}"
            )
    ...
    def test_get_main_banners_and_places_fields(self, client, news, blog, banners, plays):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert "items" in response.data[field], (
                f"Проверьте, что при GET запросе {MAIN_URL} data[{field}] " f"содержит items"
            )
    ...
    def test_get_main_video_archive_fields(self, client, news, blog, banners, plays):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert field in response.data["video_archive"], (
                f"Проверьте, что при GET запросе {MAIN_URL} " f"data[video_archive] содержит items"
            )
    ...
    def test_get_main_news_items_fields(self, client, news, blog, banners, plays):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert field in response.data["news"]["items"][0], (
                f"Проверьте, что при GET запросе {MAIN_URL} data[news][items] " f"содержит {field}"
            )
    ...
    def test_get_main_banners_items_fields(self, client, news, blog, banners, plays):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert field in response.data["banners"]["items"][0], (
                f"Проверьте, что при GET запросе {MAIN_URL} " f"data[banners][items] содержит {field}"
            )
    ...
    def test_play_count_in_short_list_in_response_matches_count_in_db(self, client, news, blog, banners, plays):
        ...
        assert objects_count_in_db == objects_count_in_response, (
            f"Проверьте, что при GET запросе {MAIN_URL} " f"возвращаются все объекты"
        )
    ...
    def test_get_main_short_list_items_fields(self, client, news, blog, banners, play):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert field in response.data["short_list"]["items"][0], (
                f"Проверьте, что при GET запросе {MAIN_URL} data[short_list]" f"[items] содержит {field}"
            )
    ...
    def test_get_main_places_items_fields(self, client, news, blog, banners, play, places):
        ...
        response = client.get(MAIN_URL)
        for field in fields:
            assert field in response.data["places"]["items"][0], (
                f"Проверьте, что при GET запросе {MAIN_URL} " f"data[places][items] содержит {field}"
            )

// Show/hide the field `pr_director_dative_name`.
// The field has to be extended in admin with CSS class `form-row field-pr_director_dative_name`.
//
// The logic:
...
jQuery(document).ready(function ($) {
    ...
    let divHideDatePrDirector = $(".form-row .field-pr_director_dative_name")

    // exit from function if there isn't selector
    if (!idIsPrDirector.length) {
        return false
    }
    // hide the field `pr_director_dative_name`.

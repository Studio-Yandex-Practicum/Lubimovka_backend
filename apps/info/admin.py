...
class FestTeamMemberAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            None,
            {
                "fields": ("pr_director_dative_name",),
                "classes": ("form-row field-pr_director_dative_name",),
            },
        ),
    )
    ...
    def save_model(self, request, obj, form, change):
        """Данные из поля 'pr_director_dative_name' проверяются и сохраняются в модели 'Setting'."""
        if form.is_valid():
            team = "fest"
            if obj.is_pr_director:
                name_director = form.cleaned_data["pr_director_dative_name"]
                FestivalTeamMember.objects.filter(is_pr_director=True).update(is_pr_director=False)
                Setting.objects.filter(settings_key="pr_director_dative_name").update(text=name_director)
            obj = form.save(commit=False)
            obj.team = team
            obj.save()
        else:
    ...
    class Media:
        """Adds a script that displays the field ```pr_director_dative_name``` if ```is_pr_director``` is selected."""
